function Skaters() {
    return (
      <div className="Skaters">
        <h2 className = "linktree-heading">Skater links:</h2>
        <ul>
            <li>
                <a href="https://docs.google.com/spreadsheets/d/1Y-ZAmVDQ7V-3OSt-miCrIgZgkw8vg61-2xo_zy_9fGg/edit?usp=sharing">
                View Attendance
                </a>
            </li>
            <li>
                <a href="https://docs.google.com/document/d/1y9OFHL4BIKDHQcQScaZcKn0KmDJy17lIxlGRJLDuYRA/edit?usp=sharing">
                Attendance policy
                </a>
            </li>
            <li>
                <a href="https://forms.gle/RnCoCBrX3ASfiXHt8">
                Record Absenses
                </a>
            </li>
            <li>
                <a href="https://docs.google.com/document/d/1CE3V7HzhCFXQMoZSVmid3woeG4RGdM7zueg5dUtjPfw/edit?usp=drive_link">
                Practice squad and BA/SB crossover policy
                </a>
            </li>
            <li>
                <a href="https://docs.google.com/document/d/1QkKml1uX75hL1yZ4olTCaBgKi4vwJAHGN7aR0_XIcW0/edit#heading=h.jt2qncihdc0h">
                WFTDA 2023 competitive rankings and system
                </a>
            </li>
        </ul>
      </div>
    );
  }
  
  export default Skaters;